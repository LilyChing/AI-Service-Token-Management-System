		...
		//Purchase token
{
	...
	for (int count = 0; count < maxInput; count++) {
		...
		if (moneySpent <= 0 || moneySpent % 2 != 0) {
			...
			int tokensPurchased = moneySpent / TOKEN_COST;
			// Update the token balance
			tokenBalance += tokensPurchased;

			cout << "Tokens purchased: " << tokensPurchased << endl;
			cout << "Current token balance: " << tokenBalance << endl;
			break;
		}
	}
	...
	// If the loop completes without valid input
	cout << "Too many invalid attempts. No tokens purchased." << endl;
	}
